...
Since $(E[X])^2=0$ (see immediately above), we have
...
[The text is an amalgam of
 Kerns\cite{kerns} and Ross{RossProb}. I don't put it in double-quotes as a citation because it would look ugly.]
...
(See derivation of $\Gamma(\alpha)$, p.\ 215  of \cite{RossProb}.)
...
\begin{definition}
\textbf{Unbiasedness}:
This is a property of the estimator. An estimator is unbiased when it gives the correct result on average (under repeated sampling). If you want to estimate $\theta$, then the expectation of the estimator $E[\hat\theta]=\theta$. 

If the estimator is not unbiased, the difference between its mean and the population quantity is the bias: Bias= $E[\hat\theta]-\theta$.
\end{definition}

%to-do: give example of bias induced in variance
...
Once we've made this assumption, an obvious question arises: what should the values of $\mu$ and $\sigma$ be? MLE serves to provide a best guess. 
...
The function $f(x)^T=(f_1(x)~f_1(x)\dots f_p(x))$ is a row vector of known functions of x.
...
Here, $f(x_1)^T = (1~x_1)$, and is the first row of the matrix $X$, and so on.
...
When we have a deterministic model  $y=\phi(f(x),\beta)=\beta_0+\beta_1x$, this implies a perfect fit to all data points. 
...
\noindent
Notice that the above shows that $\hat\beta$ is an unbiased estimate of $\beta$.
...
we feel entitled to reject the null hypothesis that $\beta_0=0$. Rejecting this null hypothesis implies that $\beta_0\neq 0$. Note that Gelman and Hill go to some trouble to get rid of the t-value and p-value; this is because at least Gelman doesn't think much of this null hypothesis testing business, with good reason---the p-value has probably caused more harm to science than any other statistical construct. 

The p-value is widely misunderstood, even by veteran scientists.  Please read our review article \cite{VasishthNicenboimStatMeth} for more details.
...
Define the \textbf{likelihood ratio test statistic} as follows. Here, $L(\theta)$ refers to the likelihood given some values of $\theta$, and 
$\ell(\theta)$ refers to the log likelihood.

\begin{equation}
\begin{split}
\lambda =& 2\times \log (L(\theta_1)/L(\theta_0)) \\
=& 2\times \{\ell (\theta_1)-\ell(\theta_0)\}\\
\lambda =& 2 \times \{\ell(\theta_1) - \ell(\theta_0)\}
\end{split}
\end{equation}
...
where r is called degrees of freedom and is the difference in the number of parameters estimated under $H_1$ and $H_0$. This is called Wilks' theorem. The proof of Wilks' theorem is fairly involved but you can find it on the internet if you are interested.
...
It should be clear that both statements are saying the same thing; in the second case, we are just subtracting the alternative hypothesis log likelihood from the null hypothesis log likelihood.
...
Recall that the residual sum of squares\marginnote{Note that I am using $e$ here, not $\epsilon$.  $e$ is computed from the estimated $\hat\beta$.} is 
